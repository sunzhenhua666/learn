...
`rte_eal_init(int argc, char **argv)` 是 DPDK（Data Plane Development Kit）库中的一个重要函数，
用于初始化 DPDK 的环境和运行时抽象层（EAL）。
该函数在 DPDK 应用程序的启动过程中是必须调用的，
它的主要功能包括：

1. **初始化 DPDK 环境：** `rte_eal_init` 函数将
   初始化 DPDK 运行时环境，包括解析命令行参数，
   初始化内部数据结构，并配置 DPDK 运行所需的环境变量。

2. **启动 EAL 抽象层：** DPDK 运行时抽象层（EAL）是 DPDK 应用程序和底层硬件平台之间的接口层。
   `rte_eal_init` 函数将启动 EAL 抽象层，
   它会执行一系列初始化步骤，如初始化内存管理、配置 PCI 设备、获取 CPU 信息等。

3. **绑定 CPU 核心：** 在初始化过程中，`rte_eal_init` 函数还
   会将 DPDK 应用程序绑定到指定的 CPU 核心，这有助于优化 DPDK 应用程序的性能，
   避免 CPU 调度带来的性能损失。

4. **创建 DPDK 线程：** DPDK 应用程序可能需要运行多个线程来处理数据包。
   `rte_eal_init` 函数将为 DPDK 应用程序创建所需的线程。

5. **设置 DPDK 内部配置：** `rte_eal_init` 函数会
   根据命令行参数和配置文件，
   设置 DPDK 的内部配置，如内存池大小、队列深度等。

6. **初始化 DPDK 日志：** DPDK 提供了日志功能，可以方便地输出调试和运行时信息。
   `rte_eal_init` 函数将初始化 DPDK 的日志模块，以便应用程序使用。

总之，`rte_eal_init` 函数是 DPDK 应用程序启动过程中的关键函数，
      它负责初始化 DPDK 运行时环境，
      并建立应用程序与底层硬件平台之间的抽象层，
      为 DPDK 应用程序提供所需的运行时支持和资源管理。
      在使用 DPDK 编写网络应用程序时，调用该函数是必不可少的第一步。
...
rte_eal_init(int argc, char **argv)
{
	...
	bool phys_addrs;
	/*
	这个结构体用于获取DPDK库的用户可见配置。
	它包含了用户可以自定义的配置项，用于初始化DPDK运行时环境。
	这些配置项通常是通过命令行参数、配置文件或者程序中的代码设置的。
	*/
	const struct rte_config *config = rte_eal_get_configuration(); // 初始化数据结构
	/*
	这个结构体则包含了DPDK库内部的配置。
	它通常不会暴露给应用程序开发者，因为其中的配置项是用于DPDK库内部的运行时优化和管理的。
	这些配置项可能会对DPDK的性能和行为产生影响，但通常不需要由应用程序直接操作。
	*/
	struct internal_config *internal_conf = eal_get_internal_configuration(); // 初始化数据结构
	...
	/*
		`__atomic_compare_exchange_n` 是用于实现原子比较和交换操作的函数。它通常用于多线程环境下，确保对共享变量的操作是原子的，
		以避免竞争条件和数据不一致性的问题。
		...
		参数说明：
		- `ptr`：指向要进行操作的变量的指针。
		- `expected`：指向用于比较的期望值的指针，函数会比较 `*expected` 和 `*ptr`。
		- `desired`：要设置的新值，如果比较成功，则将 `*ptr` 设置为此值。
		- `weak`：布尔值，用于指定是否使用弱内存序（非强内存序）进行比较和交换操作。
		- `success_memorder`：在比较成功时使用的内存序。可选值有：
		  - `__ATOMIC_RELAXED`
		  - `__ATOMIC_CONSUME`
		  - `__ATOMIC_ACQUIRE`
		  - `__ATOMIC_RELEASE`
		  - `__ATOMIC_ACQ_REL`
		  - `__ATOMIC_SEQ_CST`
		- `failure_memorder`：在比较失败时使用的内存序，可选值同上。

		函数返回值：
		- 如果比较成功，则返回 1。
		- 如果比较失败，则返回 0。

		请注意，`__atomic_compare_exchange_n` 函数可能在不同的编译器和平台上具有不同的实现方式和行为。建议查阅相关编译器文档以获取更详细的信息。 */
	...
	/*
	加载RTE_EAL_PMD_PATH目录得so库。

	RTE_EAL_PMD_PATH: 这是一个宏，
	表示 DPDK（Data Plane Development Kit）中的默认 PMD（Poll Mode Driver）路径。
	PMD 是一种 DPDK 中的设备驱动，用于与网络接口卡（NIC）、加速卡等硬件设备进行通信。
	RTE_EAL_PMD_PATH通常为： /usr/local/lib/dpdk/drivers
	*/
