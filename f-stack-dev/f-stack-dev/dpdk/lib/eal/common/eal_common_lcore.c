 ...
 * structure.
 CPU 芯片通常也被称为"Socket"或"处理器套接字"。
 每个 CPU 芯片是一个完整的物理处理单元，包含一个或多个物理 CPU 核心（Core）。
 不同的 CPU 芯片可以存在于同一台主机中，它们通常相互独立，拥有各自的内存控制器、高速缓存等资源。
 这种情况下，每个 CPU 芯片都可以被视为一个独立的 NUMA 节点。
 */
int
