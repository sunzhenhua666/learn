...
socket_id 和 core_id 是与物理硬件相关的标识符， 用于确定 lcore 在系统中的位置。
这些信息在并行编程中很重要，因为它们可以帮助你优化任务的调度，减少内存访问延迟，以及充分利用硬件资源。

cpuset 是一个字段，用于表示逻辑 CPU 核心（lcore）的亲和性（affinity），即指定了该 lcore 可以在哪些物理 CPU 上运行。
在多核心系统中，每个逻辑 CPU 核心可以与一个或多个物理 CPU 核心相关联。

通过设置亲和性，你可以控制程序的线程或任务在特定的 CPU 核心上运行，从而优化性能、减少竞争条件，并利用 CPU 缓存的局部性。

cpuset 通常是一个位向量，其中的每一位对应于系统中的一个物理 CPU 核心。如果某位被设置为 1，表示对应的 CPU 核心是允许运行该 lcore 的核心。如果某位被设置为 0，表示对应的 CPU 核心不应运行该 lcore。
例如，如果系统有 4 个物理 CPU 核心，并且 cpuset 被设置为 0001，那么该 lcore 将被限制在第一个物理核心上运行。如果设置为 1110，那么该 lcore 可以在后三个物理核心上运行。
通过设置亲和性，你可以优化线程或任务的运行位置，从而最大限度地提高性能。这在需要控制线程之间的竞争、利用 CPU 缓存、避免跨 NUMA 节点访问等情况下特别有用。但需要注意的是，过度设置亲和性也可能导致资源分配不均衡，应根据实际情况进行调整。
 */
struct lcore_config {
...
main_lcore: 主逻辑核心的 ID。通常是第一个被初始化的逻辑核心，用于控制整个 DPDK 应用程序的初始化和关闭。

lcore_count: 可用逻辑核心的数量。

numa_node_count: 检测到的 NUMA 节点的数量。

numa_nodes[RTE_MAX_NUMA_NODES]: 检测到的 NUMA 节点的列表。NUMA 节点是一种非均匀内存访问架构，用于优化内存访问和数据局部性。

service_lcore_count: 可用服务核心的数量。服务核心通常用于处理非数据平面任务。

lcore_role[RTE_MAX_LCORE]: 每个核心的角色，例如主逻辑核、次要逻辑核、服务核等。

process_type: 进程类型，可以是主进程或次要进程。这在多进程应用程序中很有用。

iova_mode: 内存映射模式，可以是 PA（物理地址）或 VA（虚拟地址）映射模式。这涉及到 DPDK 对物理地址和虚拟地址的管理方式。

mem_config: 指向内存配置的指针，这可能在多个 DPDK 实例之间共享。它用于配置内存池等内存相关的设置。
 */
struct rte_config {
	uint32_t main_lcore;         /**< Id of the main lcore *///主逻辑核 ID
	uint32_t lcore_count;        /**< Number of available logical cores. */
	uint32_t numa_node_count;    /**< Number of detected NUMA nodes. */
	uint32_t numa_nodes[RTE_MAX_NUMA_NODES]; /**< List of detected NUMA nodes. */// 检测到的 NUMA 节点列表
